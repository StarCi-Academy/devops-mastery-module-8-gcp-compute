#!/usr/bin/env bash
# verify-zero-cost.sh — after `terraform destroy`, assert no LB / Cloud Armor /
# CDN / DNS resource is left behind still carrying this lesson's label. Run with
# the lesson slug as the first arg (defaults to the lab slug).
set -euo pipefail

LESSON_SLUG="${1:-3-cloud-armor-cdn-and-edge}"
...
# Cloud Asset inventory across all resource types in one query.
